...
class CimType(models.Model):
    ...
    def _compute_notes_text(self):
        ...
        for record in self:
            notes_text = ""
            if record.notes:
                notes_text = model_converter.text_from_html(record.notes, 50, 150)
        ...
        for record in self:
            if record.is_standard:
                raise exceptions.UserError(
                    _("It is not possible to remove " "a 'STANDARD' complaint type.")
                )
        res = super(CimType, self).unlink()
        return res

    def action_show_complaints(self):
        self.ensure_one()
        current_complaint_type = self
        id_tree_view = self.env.ref("cim_channel.cim_view_tree").id
        id_form_view = self.env.ref("cim_channel.cim_view_form").id
        search_view = self.env.ref("cim_channel.cim_view_search")
        custom_context = {"default_complaint_type_id": current_complaint_type.id}
        act_window = {
