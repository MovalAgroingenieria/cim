...
class CimComplaint(models.Model):
    ...
    def _default_tracking_code(self):
        ...
        while not tracking_code_ok:
            tracking_code = "".join(
                random.choice(characters) for _ in range(self._size_tracking_code)
            )
            repeated_complaints = self.search([("tracking_code", "=", tracking_code)])
    ...
    def _default_setted_sequence(self):
        ...
        sequence_complaint_code_id = config.get_param(
            "cim_channel.sequence_complaint_code_id", False
        )
    ...
    state = fields.Selection(
        selection=[
            ...
            ("02_admitted", "Admitted"),
            ("03_in_progress", "In progress"),
            ("04_ready", "Ready"),
            ("05_resolved", "Resolved"),
        ],
        default="01_received",
        required=True,
        index=True,
    ...
    communication_ids = fields.One2many(
        ...
        comodel_name="cim.communication",
    ...
    deadline_state = fields.Selection(
        ...
        selection=[
            ("01_on_time", "Instruction on time"),
            ("02_upcoming_expiration", "Instruction deadline soon to expire"),
            ("03_expirated", "Instruction period expirated"),
            ("04_extended", "Extended instruction on time"),
            (
                "05_extended_upcoming_expiration",
                "Extended instruction deadline soon to expire",
            ),
            ("06_extended_expirated", "Extended instruction period expirated"),
            ("07_finished", "Instruction finished"),
            ("99_rejected", "Instruction rejected"),
        ],
        default="01_on_time",
    ...
    icon_warning = fields.Binary(
        string="Icon for warnings", compute="_compute_icon_warning"
    )
    ...
    def _compute_is_anonymous(self):
        for record in self:
            ...
            if (
                ...
                or record.complainant_vat
                or record.complainant_phone
            ):
                is_anonymous = False
            record.is_anonymous = is_anonymous

    def _compute_user_in_group_cim_settings(self):
        user_in_group_cim_settings = self.env.user.has_group(
            "cim_channel.group_cim_settings"
        )
        for record in self:
    ...
    def _compute_param_acknowledgement_period(self):
        ...
        param_acknowledgement_period = config.get_param(
            "cim_channel.acknowledgement_period", False
        )
    ...
    def _compute_param_notice_period(self):
        config = self.env["ir.config_parameter"].sudo()
        param_notice_period = config.get_param("cim_channel.notice_period", False)
    ...
    def _compute_param_deadline(self):
        config = self.env["ir.config_parameter"].sudo()
        param_deadline = config.get_param("cim_channel.deadline", False)
    ...
    def _compute_param_deadline_extended(self):
        ...
        param_deadline_extended = config.get_param(
            "cim_channel.deadline_extended", False
        )
    ...
    def _compute_param_email_for_notice(self):
        config = self.env["ir.config_parameter"].sudo()
        param_email_for_notice = config.get_param("cim_channel.email_for_notice", False)
    ...
    def _compute_deadline_date(self):
        for record in self:
            ...
            deadline_date = (
                record.creation_date + relativedelta(months=instruction_months, days=-1)
            ).strftime("%Y-%m-%d")
        ...
        for record in self:
            ...
            if record.is_rejected:
                ...
                deadline_date = (
                    ...
                    + relativedelta(months=months_deadline, days=-1)
                ).strftime("%Y-%m-%d")
                if current_date <= deadline_date:
                    ...
                    deadline_notice = (
                        ...
                        + relativedelta(days=-record.param_notice_period + 1)
                    ).strftime("%Y-%m-%d")
    ...
    def _search_is_acknowledgement_expired(self, operator, value):
        ...
        if acknowledgement_period >= 0:
            sql_statement = """
                ...
                AND state = %s
                AND current_date - %s >= creation_date
            """
            self.env.cr.execute(sql_statement, ("01_received", acknowledgement_period))
        else:
            sql_statement = """
                SELECT id FROM cim
                WHERE is_rejected = false
                AND state = %s
            """
            self.env.cr.execute(sql_statement, ("01_received",))
    ...
    def _compute_icon_warning(self):
        image_path_is_acknowledgement_expired_no = modules.module.get_resource_path(
            "cim_channel", "static/img", "icon_ontime.png"
        )
        image_path_is_acknowledgement_expired_yes = modules.module.get_resource_path(
            "cim_channel", "static/img", "icon_expirated.png"
        )
        image_path_is_rejected = modules.module.get_resource_path(
            "cim_channel", "static/img", "icon_rejected.png"
        )
        for record in self:
            icon_warning = None
            image_path = None
    ...
    def _compute_decrypted_complainant_data(self):
        for record in self:
            ...
            if decrypted_complainant_vat:
                decrypted_complainant_data = (
                    decrypted_complainant_data + decrypted_complainant_vat + "\n"
                )
            if decrypted_complainant_email:
                decrypted_complainant_data = (
                    decrypted_complainant_data + decrypted_complainant_email + "\n"
                )
            if decrypted_complainant_phone:
                decrypted_complainant_data = (
                    decrypted_complainant_data + decrypted_complainant_phone + "\n"
                )
    ...
    def _compute_automatic_email_state(self):
        ...
        automatic_email_state = config.get_param(
            "cim_channel.automatic_email_state", False
        )
        for record in self:
        ...
        for record in self:
            if record.state != "01_received" and record.is_rejected:
                raise exceptions.ValidationError(
                    _(
                        "It is only possible to reject a complaint if its "
                        "state is 'Received'."
                    )
                )
    ...
    def _check_investigating_user_id(self):
        for record in self:
            if (not record.investigating_user_id) and (
                ...
                raise exceptions.ValidationError(
                    _(
                        "If the complaint is ready or resolved, then it is "
    ...
    def _check_measures_taken(self):
        for record in self:
            if (record.state in ("04_ready", "05_resolved")) and (
    ...
    def _check_resolution_text(self):
        for record in self:
            if record.state == "05_resolved" and (not record.resolution_text):
                raise exceptions.ValidationError(
                    _(
                        "If the complaint is resolved, then it is mandatory "
    ...
    def name_get(self):
        result = []
        for record in self:
            display_name = record.name + " (" + record.complaint_type_id.name + ")"
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if sequence_complaint_code_id:
                ...
                sequence_complaint_code = model_ir_sequence.browse(
                    int(sequence_complaint_code_id)
                )
            ...
            if ("name" not in vals) or (not vals["name"]):
                raise exceptions.ValidationError(
                    _("It is mandatory to enter a " "code for the new complaint.")
                )
            ...
            if new_complaint.complainant_email:
                ...
                try:
                    mail_template_tracking_code = self.env.ref(
                        "cim_channel." "mail_template_tracking_code"
                    ).sudo()
                except Exception:
                    mail_template_tracking_code = None
                if mail_template_tracking_code:
                    ...
                    mail_template_tracking_code.with_context(lang=user_lang).send_mail(
    ...
    def _process_vals(self, vals, is_create=False):
        if vals:
            self._process_state_vals(vals)
            vals = self._process_document_vals(vals, is_create)
            self._process_default_vals(vals, is_create)
            self._process_document_name_cleanup(vals, is_create)
            self._process_complaint_time_vals(vals)
            self._process_encrypted_fields(vals)
            self._process_language_vals(vals)
        return vals

    def _process_state_vals(self, vals):
        """Process state-related values"""
        if "state" in vals and vals["state"]:
            if vals["state"] == "05_resolved":
                vals["resolution_date"] = datetime.now().strftime("%Y-%m-%d")
            else:
                vals["resolution_date"] = None

    def _process_document_vals(self, vals, is_create):
        """Process document-related values"""
        document_fields = [f"document_{i:02d}" for i in range(1, 7)]
        has_documents = any(field in vals for field in document_fields)
        if (is_create or len(self) == 1) and has_documents:
            return self._compact_document_fields(vals, is_create)
        return vals

    def _process_default_vals(self, vals, is_create):
        """Process default values for creation"""
        if is_create:
            if "complaint_type_id" not in vals or not vals["complaint_type_id"]:
                vals["complaint_type_id"] = self.env.ref(
                    "cim_channel.cim_complaint_type_other"
                ).id
            if "link_type_id" not in vals or not vals["link_type_id"]:
                vals["link_type_id"] = self.env.ref(
                    "cim_channel.cim_link_type_other"
                ).id

    def _process_document_name_cleanup(self, vals, is_create):
        """Clean up document name fields when document is not present"""
        if is_create:
            document_pairs = [
                (f"document_{i:02d}", f"document_{i:02d}_name") for i in range(1, 7)
            ]
            for doc_field, name_field in document_pairs:
                if name_field in vals and doc_field not in vals:
                    vals.pop(name_field)

    def _process_complaint_time_vals(self, vals):
        """Process complaint frequency and time values"""
        if "complaint_frequency" in vals and vals["complaint_frequency"]:
            if vals["complaint_frequency"] != "02_specific_day":
                ...
                if "complaint_time" not in vals or not vals["complaint_time"]:
                    vals["complaint_time"] = datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )

    def _process_encrypted_fields(self, vals):
        ...
        for field in encrypted_fields:
            if field in vals and vals[field]:
                vals[field] = self.encrypt_data(vals[field], self._cipher_key)

    def _process_language_vals(self, vals):
        """Process language-related values"""
        if "complaint_lang" not in vals or vals["complaint_lang"] == "":
    ...
    def _compact_document_fields(self, vals, is_create=False):
        ...
        if any(field in vals for field in document_fields):
            document_values = self._collect_document_values(vals, is_create)
            filtered_values = [v for v in document_values if v["document"]]
            self._reassign_document_fields(vals, filtered_values)
            self._clear_empty_document_fields(vals, len(filtered_values))
        return vals

    def _collect_document_values(self, vals, is_create):
        """Collect all document values from vals and current record"""
        document_values = []
        for i in range(1, 7):
            doc_field = f"document_{i:02d}"
            name_field = f"document_{i:02d}_name"

            if doc_field in vals:
                document = vals[doc_field]
                document_name = vals.get(name_field)
            elif not is_create:
                document = getattr(self, doc_field, None)
                document_name = getattr(self, name_field, None)
            else:
                document = None
                document_name = None
    ...
    def _reassign_document_fields(self, vals, filtered_values):
        """Reassign document fields to eliminate gaps"""
        for i, value in enumerate(filtered_values):
            doc_field = f"document_{i+1:02d}"
            name_field = f"document_{i+1:02d}_name"
            vals[doc_field] = value["document"]
            vals[name_field] = value["document_name"]

    def _clear_empty_document_fields(self, vals, num_values):
        """Clear document fields beyond the number of actual values"""
        if num_values < self.MAX_DOCUMENTS:
            for i in range(num_values, self.MAX_DOCUMENTS):
                doc_field = f"document_{i+1:02d}"
                name_field = f"document_{i+1:02d}_name"
                vals[doc_field] = None
                vals[name_field] = None
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type in ["form", "tree"]:
            if not self.env.user.has_group("cim_channel.group_cim_settings"):
                if view_type == "tree":
    ...
    def fields_get(self, *args, **kwargs):
        ...
        for field_to_hide in fields_to_hide:
            if field_to_hide in res:
                ...
                if "searchable" in data_of_field and data_of_field["searchable"]:
        ...
        for record in self:
            if not (record.is_delegated or record.is_rejected):
                raise exceptions.UserError(
                    _("It is not possile to delete a in force complaint.")
                )
    ...
    def action_get_communications(self):
        ...
        id_tree_view = self.env.ref("cim_channel.cim_communication_view_tree").id
        id_form_view = self.env.ref("cim_channel.cim_communication_view_form").id
        search_view = self.env.ref("cim_channel.cim_communication_view_search")
        act_window = {
            "type": "ir.actions.act_window",
            "name": _("Communications"),
            "res_model": "cim.communication",
            "view_type": "form",
            "view_mode": "form,tree",
            ...
            "context": {
                "default_complaint_id": current_complaint.id,
    ...
    def action_undo(self):
        ...
        if (
            self.state != "01_received"
            and self.state != "04_ready"
            and self.automatic_email_state
        ):
            self._create_communication()
    ...
    def action_complainant_data(self):
        self.ensure_one()
        if self.is_delegated and self.state != "05_resolved" and (not self.is_rejected):
            act_window = {
                "type": "ir.actions.act_window",
                "name": _("Complaint") + " : " + self.name,
    ...
    def _create_communication(self, extension=False):
        ...
        if extension:
            ...
            description = (
                _(
                    "Due to the special complexity of the case, "
            ...
            if self.is_rejected:
                ...
                if self.state == "02_admitted":
                    issue = _("Acknowledgement of receipt of the complaint filed")
                    description = (
                        _(
                            "You have received an acknowledgement of receipt of "
        ...
        new_communication = (
            self.env["cim.communication"].with_context(from_backend=True).create(vals)
        )
    ...
    def create_initial_communication(self):
        for record in self:
            ...
            self.env["cim.communication"].create(vals)
...
class CimComplaintCommunication(models.Model):
    ...
    def _default_communication_number(self):
        ...
        try:
            ...
            id_of_complaint = 0
        if id_of_complaint:
            last_communication = self.env["cim.communication"].search(
                [("complaint_id", "=", id_of_complaint)],
                limit=1,
                order="communication_number desc",
            )
    ...
    icon_inputoutput = fields.Binary(
        string="Icon for input/output", compute="_compute_icon_inputoutput"
    )
    ...
    _sql_constraints = [
        ...
        ("name_unique", "UNIQUE (name)", "Existing communication."),
    ]
    ...
    def _compute_automatic_email_validate_com(self):
        ...
        automatic_email_validate_com = config.get_param(
            "cim_channel.automatic_email_validate_com", False
        )
        for record in self:
    ...
    def _compute_automatic_email_complainant_com(self):
        ...
        automatic_email_complainant_com = config.get_param(
            "cim_channel.automatic_email_complainant_com", False
        )
        for record in self:
            record.automatic_email_complainant_com = automatic_email_complainant_com
    ...
    def _compute_icon_inputoutput(self):
        image_path_input = modules.module.get_resource_path(
            "cim_channel", "static/img", "icon_right.png"
        )
        image_path_output = modules.module.get_resource_path(
            "cim_channel", "static/img", "icon_left.png"
        )
        for record in self:
    ...
    def _compute_description_as_html(self):
        for record in self:
            ...
            if record.description:
                description_as_html = self._convert_text_to_html(record.description)
    ...
    def _check_complaint_id(self):
        for record in self:
            if (not record.complaint_id) or (not record.communication_number):
                raise exceptions.ValidationError(
                    _("It is mandatory to enter the complaint of the " "communication.")
                )

    def name_get(self):
        result = []
        # communication_show_complete_code
        communication_show_complete_code = self.env.context.get(
            "communication_show_complete_code", True
        )
        for record in self:
            ...
            if record.complaint_id and record.communication_number:
                if communication_show_complete_code:
                    ...
                    display_name = _("#") + " " + str(record.communication_number)
            result.append((record.id, display_name))
        return result
                ...
                for complaint in complaints or []:
                    decrypted_tracking_code_of_complaint = (
                        model_cim_complaint.decrypt_data(
                            complaint.tracking_code, model_cim_complaint._cipher_key
                        )
                    )
                    if (
    ...
    def _process_vals(self, vals, is_create=False):
        if vals:
            self._process_communication_document_cleanup(vals, is_create)
            vals = self._process_communication_documents(vals, is_create)
            self._process_communication_number(vals, is_create)
        return vals

    def _process_communication_document_cleanup(self, vals, is_create):
        """Clean up document name fields when document is not present"""
        if is_create:
            document_pairs = [
                (f"document_{i:02d}", f"document_{i:02d}_name") for i in range(1, 7)
    ...
    def _process_communication_documents(self, vals, is_create):
        """Process document fields for communication"""
        document_fields = [f"document_{i:02d}" for i in range(1, 7)]
        has_documents = any(field in vals for field in document_fields)
        if (is_create or len(self) == 1) and has_documents:
            return self._compact_document_fields(vals, is_create)
    ...
    def _process_communication_number(self, vals, is_create):
        """Process communication number for new communications"""
        if is_create and "complaint_id" in vals and vals["complaint_id"]:
            ...
            if last_communication:
                last_communication = last_communication[0]
                communication_number = last_communication.communication_number + 1
    ...
    def _clear_empty_document_fields(self, vals, num_values):
        """Clear document fields beyond the number of actual values"""
        max_documents = CimComplaint.MAX_DOCUMENTS
        if num_values < max_documents:
            for i in range(num_values, max_documents):
                doc_field = f"document_{i+1:02d}"
                name_field = f"document_{i+1:02d}_name"
                vals[doc_field] = None
                vals[name_field] = None
    ...
    def unlink(self):
        for record in self:
            if record.state != "01_draft":
                raise exceptions.UserError(
                    _("It is not possile to delete a validated communication.")
                )
    ...
    def action_send_mail(self):
        ...
        if (
            communication
            and communication.state == "02_validated"
            and communication.with_email
        ):
            communication.send_mails()
        ...
        for record in self:
            ...
            if record.complaint_id.complainant_email:
                decrypted_complainant_email = record.complaint_id.decrypt_data(
                    ...
                    record.complaint_id._cipher_key,
                ).decode("utf-8")
            if record.state == "02_validated" and decrypted_complainant_email:
                text_for_log_ok = _("Email sent. Destination:")
                text_for_log_fail = _(
                    "ERROR, the email could not be sent. " "Destination:"
                )
                mail_ok = self._send_communication(record)
                if mail_ok:
                    record.is_sent = True
                    message_ok = text_for_log_ok + " " + decrypted_complainant_email
                    record.message_post(body=message_ok)
                else:
                    message_fail = text_for_log_fail + " " + decrypted_complainant_email
    ...
    def _send_communication(self, communication):
        ...
        try:
            mail_template_communication = self.env.ref(
                "cim_channel." "mail_template_communication"
            ).sudo()
        except Exception:
            mail_template_communication = None
        if mail_template_communication:
            ...
            try:
                mail_template_communication.with_context(lang=user_lang).send_mail(
    ...
    def _send_notice(self, communication):
        ...
        try:
            mail_template_notice = self.env.ref(
                "cim_channel." "mail_template_notice"
            ).sudo()
        except Exception:
            mail_template_notice = None
        if mail_template_notice:
            ...
            if communication.complaint_id.complaint_lang:
                user_lang = communication.complaint_id.complaint_lang
            try:
                mail_template_notice.with_context(lang=user_lang).send_mail(
                    communication.id, force_send=True
                )
            except Exception:
                resp = False
